...
{
    return 1.0h - OneMinusReflectivityFromMetallic(_MetallicScale);
}
...
VertexOutputBaseSimple vertForwardBaseSimple (VertexInput v)
{
	...
	#if defined(_LEAF_ON)
	v.vertex = AnimateVertex(v.vertex, v.normal);
	#endif
    ...
    #if !GLOSSMAP
        o.eyeVec.w = saturate(_GlossScale + UNIFORM_REFLECTIVITY()); // grazing term
    #endif
...
FragmentCommonData FragmentSetupSimple(VertexOutputBaseSimple i)
{
    ...
    #ifdef _NORMALMAP
        s.tangentSpaceNormal =  NormalInTangentSpace(i.tex,i.tex);
    #else
