...
{				
	...
	float lerp1 = lerp(
		dot(random(pi + float2(0.0, 0.0)), pf - float2(0.0, 0.0)),
...
)
{
	...
	v.vertex = NoCollisionAnimateVertex(v.vertex, v.normal, animParams);
	#endif
...
) : SV_Target
{
        ...
        half alpha = 0;
        half cutoff = 0.5;
	    #if defined(_LEAF_ON) && defined (SHADER_API_MOBILE)
            half temp = 1 - (distance(i.tex.xy, half2(0.5, 0.5)) * 1.5);
            half2 uv = i.tex.xy * half2(60, 60);
            alpha = perlinNoise(uv) * temp;
            cutoff = 0.05;
        #else
