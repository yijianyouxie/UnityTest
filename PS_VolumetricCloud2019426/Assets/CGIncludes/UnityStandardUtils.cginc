...
half3 UnpackScaleNormalRGorAG(half4 packednormal, half bumpScale)
{
	...
	// 在非pc平台也使用两通道法线，避免由于法线贴图z通道错误造成的误差
	half3 normal;
	...
	/**
	  ...
	  //  #define StereographicProjection

		#if defined(StereographicProjection)
			half pX = normal.x / ( 1 + normal.z );
			half pY = normal.y / ( 1 + normal.z );

			half denom = 2 / ( 1 + pX * pX + pY * pY );
			normal.x = pX * denom;
			normal.y = pY * denom;
			normal.z = denom - 1;
		#endif
