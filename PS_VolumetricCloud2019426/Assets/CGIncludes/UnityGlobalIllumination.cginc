...
inline UnityGI UnityGI_Base(UnityGIInput data, half occlusion, half3 normalWorld)
{
    ...
    //o_gi.indirect.diffuse += (4.59 - o_gi.indirect.diffuse) * _GIDiffuseScale;
    o_gi.indirect.diffuse *= occlusion;
    return o_gi;
}

inline half3 UnityGI_IndirectSpecular(UnityGIInput data, half occlusion, Unity_GlossyEnvironmentData glossIn,half3 normalWorld)
{
    half3 specular;
		...
		float TopN = (dot(normalWorld, float3(0, 1, 0)) + 1)*0.5;
		half3 ambientColor = lerp(half3(0.63, 0.63, 0.63), half3(0.27, 0.27, 0.27), TopN);
		
		specular = ambientColor;//原来unity_IndirectSpecColor是根据天空盒颜色计算,但是咱们没有天空盒颜色,是黑的,导致金属发黑
		//UNITY_LIGHTMODEL_AMBIENT的颜色主要是氛围会比较偏离金属,现在的做法是用了一个比较相近的颜色做替代
		//specular = UNITY_LIGHTMODEL_AMBIENT.xyz;
    #else
...
inline half3 UnityGI_IndirectSpecular(UnityGIInput data, half occlusion, half3 normalWorld, Unity_GlossyEnvironmentData glossIn)
{
    ...
    return UnityGI_IndirectSpecular(data, occlusion, glossIn, normalWorld);
}
...
inline UnityGI UnityGlobalIllumination (UnityGIInput data, half occlusion, half3 normalWorld, Unity_GlossyEnvironmentData glossIn)
{
    ...
    o_gi.indirect.specular = UnityGI_IndirectSpecular(data, occlusion, glossIn, normalWorld);
    return o_gi;
}
