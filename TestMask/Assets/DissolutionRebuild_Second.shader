...
Shader "TLStudio/Character/Character1\MoonFadeIn" 
{
	Properties
	{
		...
		_Speed("_Speed", Range(0, 100)) = 0 //速度
		_SpeedAlpha("_SpeedAlpha", Float) = 0.1 //显现速度
		_HiddenSpeedAlpha("alphaHideSpeed", Float) = 0.1 //消失速度
		_Amount("_Amount", Range(0, 1)) = 0 // 溶解度 
		_DisappearOffset("_DisappearOffset", Range(-10, 10)) = 0  //控制真身 多久显示出来  初始值越小  越慢出来
		_DisappearOffset2("_DisappearOffset2(DissolveRebuildBeginOffset-0.15)", Range(-10, 10)) = -0.15  //控制半透 多久消失  需要配合offset1 基本相差0.15左右的值  
		_DissSize("DissolveArea", Range(0, 1)) = 0.1 // 溶解范围大小 
		_DissColor("DissolveMainColor", Color) = (1,1,1,1) // 溶解颜色 
		_AddColor("DissolveSecondColor", Color) = (1,1,1,1) // 改色与溶解色融合形成开始色 
		...
		_ClipRange("_ClipRange", Range(-10.0, 10.0)) = -10
	}
	...
	SubShader
	{
		...
		Pass
		{
			...
			VertexOutput vert(VertexInput v) 
			{
				...
				//溶解重塑部分
				//_DisappearOffset += _Speed * (_Time.x - _TimeSinceLevelLoad);
				//_DisappearOffset += _Speed * (_Time.x - 0) / 20;
				_DisappearOffset += 100;
				//o.uv0.z = _DisappearOffset - (v.vertex.x / v.vertex.w*0.5 + 0.5);  //(vertex -1,1)
				//o.uv0.z = _DisappearOffset - (v.vertex.x*0.5 + 0.5);  //(vertex -1,1)
				//o.uv0.z = _DisappearOffset - (-v.vertex.z / v.vertex.w*0.5  + 0.5);  //(vertex -1,1)//这里的v.vertex.y表示沿物体的y坐标轴。x的话就是沿物体的x轴
				
				//分象限
				if (o.uv0.x >= 0.5 && o.uv0.y >= 0.5)
				{
					//第一象限
					o.uv0.z = _DisappearOffset - 0.5;
				}
				else if (o.uv0.x > 0.5 && o.uv0.y < 0.5)
				{
					//第二象限
					o.uv0.z = _DisappearOffset - 0.2;
				}
				else if (o.uv0.x < 0.5 && o.uv0.y < 0.5)
				{
					//第三象限
					o.uv0.z = _DisappearOffset - 0.3;
				}
				else 
				if (o.uv0.x < 0.5 && o.uv0.y > 0.5)
				{
					//第四象限
					o.uv0.z = _DisappearOffset - 0.4;
				}
				else
				{
					o.uv0.z = 0;
