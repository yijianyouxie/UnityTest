...
Shader "DCG/Water Shader/Water Surface OpenGL" {
    ...
    SubShader {
        ...
        Pass {
			...
			#include "../../../CLX/Shader/CYUnityCG.cginc"
            ...
            VertexOutput vert (VertexInput v) {
				...
				float3 dis = o.posWorld.xyz - _WorldSpaceCameraPos;
				CY_TRANSFER_FOG(o, dis.xyz, o.posWorld.y);
...
/////// Vectors:
                ...
                float3 lightDirection0 = normalize(_WorldSpaceLightPos0.xyz);
                float3 lightColor = _LightColor0.rgb;
////// Lighting:
                float attenuation = 1;
                float node_8267 = 1.0;
...
/////// Vectors:
                ...
                float3 normalDirection = normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
                float3 viewReflectDirection = reflect( -viewDirection, normalDirection );
                float3 lightDirection0 = normalize(lerp(_WorldSpaceLightPos0.xyz, _WorldSpaceLightPos0.xyz - i.posWorld.xyz,_WorldSpaceLightPos0.w));
                float3 lightColor = _LightColor0.rgb;
////// Lighting:
                float attenuation = 1;//LIGHT_ATTENUATION(i);
                float node_8267 = 1.0;
                float4 node_7838 = tex2D(_ReflectionTex,TRANSFORM_TEX(sceneUVs.rg, _ReflectionTex));
                float4 node_9204 = _Time + _TimeEditor;
                float node_827 = (_FoamSpeed*node_9204.r);
