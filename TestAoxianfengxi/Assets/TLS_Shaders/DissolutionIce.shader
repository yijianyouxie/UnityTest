...
Shader "TLStudio/FX/DissolutionIce"
{
	//适用于那种直线的模型，比如直的桥
	//外部传入一个位置点，通过将此点转换为模型坐标后，按照某个轴进行比较
	Properties
	{
...
#line 12 ""
		...
		// with lightmaps:
			...
			v2f_surf vert_surf(appdata_full v) {
				...
				o.pack0.z = v.vertex.y;
				o.pack0.w = mul(unity_WorldToObject, float4(_TargetPosition.xyz, 1)).y;
		...
		Pass
		{
			...
			v2f vert(appdata v)
			{
				...
				o.uv.z = v.vertex.y;
				o.uv.w = mul(unity_WorldToObject, float4(_TargetPosition.xyz, 1)).y;
				return o;
			}

			fixed4 frag(v2f i) : SV_Target
			{
				float delta = i.uv.w - i.uv.z;
