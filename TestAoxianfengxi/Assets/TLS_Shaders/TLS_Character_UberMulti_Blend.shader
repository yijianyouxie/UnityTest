...
Shader "TLStudio/Character/UberMulti_Blend" {
	Properties{
		...
		[Sub(g1)]_BubbleFrequency("泡泡频率", Range(0, 10)) = 0
		[Sub(g1)]_BubbleAmplitute("泡泡振幅", Range(0, 2)) = 0
		[Title(g1, _MaskTex)]
    ...
    SubShader {
        ...
        Pass {
			...
			#pragma exclude_renderers xbox360 ps3 flash d3d11_9x//why Too many math instructions for SM2.0 (70 needed, max is 64). 
            #pragma vertex vert
			...
			uniform fixed _BubbleFrequency;
			uniform fixed _BubbleAmplitute;
            ...
            VertexOutput vert (VertexInput v) {
                ...
                //#endif
				float3 value = (cos(_BubbleFrequency * v.vertex.y + _Time.y) * 0.015 + sin(_BubbleFrequency * v.vertex.y + _Time.y) * 0.005).xxx;
				v.vertex.xyz += value * _BubbleAmplitute;
...
/// Glitter MatCap:
				...
				finalColor.rgb = GetDynamicPointLightColor(finalColor.rgb, worldPos, normalDir);
