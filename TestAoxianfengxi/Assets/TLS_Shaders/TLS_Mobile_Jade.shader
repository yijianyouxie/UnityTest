    ...
    SubShader {
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                o.uv0 = v.texcoord0;
                //将法线规格化操作放到顶点渲染
                o.normalDir = UnityObjectToWorldNormal(v.normal);// normalize(mul(float4(v.normal, 0.0), _World2Object).xyz);
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                fixed3 normalLocal = UnpackNormal(tex2D(_normal,TRANSFORM_TEX(i.uv0, _normal)));
				fixed3 normalDirection = normalize(mul( normalLocal, tangentTransform ));
				fixed3 lightDirection = normalize(_lightDrectiotn.xyz);
				...
				fixed3 halfDirection = normalize(viewDirection);
				//对颜色贴图进行采样
				fixed4 _main_var = tex2D(_MainTex, TRANSFORM_TEX(i.uv0, _MainTex));
				...
				fixed3 directDiffuse = max(fixed3(0.0,0.0,0.0), NdotLWrap + _3S);
