...
SubShader {
	Tags {"Queue"="AlphaTest+50" "IgnoreProjector"="True" "RenderType"="Geometry" "ShadowProjector" = "true"}
	LOD 150
    Cull Off
...
CGPROGRAM
#pragma skip_variants DYNAMICLIGHTMAP_ON LIGHTMAP_SHADOW_MIXING SHADOWS_SHADOWMASK FOG_EXP
// compile directives
...
fixed _ReflectionIntension;
 fixed4 LightingHairSpecular (SurfaceOutput s, fixed3 lightDir, fixed3 viewDir, fixed atten)
{
        fixed3 h = normalize (lightDir + viewDir);
        fixed diff = max (0.2, dot (s.Normal, lightDir));
		fixed nh = max (0, dot (s.Normal, h));
        fixed rim = 1- max (0, dot (s.Normal,viewDir));
...
v2f_surf vert_surf (appdata_full v) {
  ...
  #endif
  fixed3 worldN = UnityObjectToWorldNormal(SCALED_NORMAL);// mul((float3x3)_Object2World, SCALED_NORMAL);
  #ifdef LIGHTMAP_OFF
  o.normal = worldN;
  #endif
  #ifdef LIGHTMAP_OFF
  fixed3 viewDirForLight = WorldSpaceViewDir( v.vertex );
  o.viewDir = viewDirForLight;
  #endif

  // SH/ambient and vertex lights
  #ifdef LIGHTMAP_OFF
  o.vlight = ShadeSH9(fixed4(normalize(mul(unity_ObjectToWorld, float4(v.normal,0)).xyz) * 1.0,1)); 
  #endif // LIGHTMAP_OFF
...
fixed4 frag_surf (v2f_surf IN) : SV_Target {
  ...
  // lightmaps:
      ...
      fixed3 lm = LightingLambert_DirLightmap(o, lmtex, lmIndTex, 0).rgb;
