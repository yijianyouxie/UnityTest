...
Shader "TLStudio/Transparent/AlphaBlend" {
	Properties{
...
//		[NoScaleOffset] _SnowTex ("Snow Tex", 2D) = "white" {}
		...
		SubShader{
		Tags{ "Queue"="Transparent-101"  "IgnoreProjector" = "False" "RenderType" = "Transparent" }
		LOD 200
		Cull Off
		...
		CGPROGRAM
		#pragma target 3.0
		#pragma skip_variants FOG_EXP DIRLIGHTMAP_COMBINED VERTEXLIGHT_ON SHADOWS_CUBE
		// compile directives
...
#line 14 ""
	...
	if (EnableGlobalSpecular > 0)
	{
		...
		float3 finalGlobalSpecular = /*o.Alpha * */globalSpecular * GlobalSpecularIntensity * FresnelTerm(GlobalSpecularColor.xyz, lhCustom) * pow((1 - nv), 2);
		c.rgb = c.rgb + finalGlobalSpecular * 0.3f;
	}
