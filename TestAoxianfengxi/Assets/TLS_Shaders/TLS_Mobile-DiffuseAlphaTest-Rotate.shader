...
Shader "TLStudio/Transparent/Cutout-Rotate" {
	Properties{
		_Color("Color",Color) = (1.0, 1.0, 1.0, 1.0)
		_MainTex("Base (RGB)", 2D) = "white" {}
		_Cutoff("Alpha cutoff", Range(0,1)) = 0.5
		_RSpeed("_RSpeed", Range(0,10)) = 1.0
	}
		SubShader{
...
#line 14 ""
	...
	void surf(Input IN, inout SurfaceOutput o) {
		...
		uv = float2(uv.x * cos(_RSpeed * _Time.y) - uv.y * sin(_RSpeed * _Time.y),
					uv.x * sin(_RSpeed * _Time.y) + uv.y * cos(_RSpeed * _Time.y));
