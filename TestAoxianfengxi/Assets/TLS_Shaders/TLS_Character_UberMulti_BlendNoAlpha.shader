...
Shader "TLStudio/Character/UberMulti_BlendNoAlpha" {
    Properties {
		...
		[MaterialEnum(Off, 0, On, 1)]_ZWrite("    Z写入模式", Int) = 1
...
/// Glitter MatCap:
				...
				if (_MainTexColor.a <= 0.1)
				{
					float maxValue = max(max(finalColor.r, finalColor.g), finalColor.b);
					clip(maxValue - 0.15);
				}
