...
void HeightTransferFog(inout float4 fogCoord, float4 pos, float4 objPos)
{
	...
	/*float4 noise = tex2Dlod(_NoiseTex, float4(worldPos.x/ NoiseScale + _Time.y * NoiseSpeed, worldPos.y/ NoiseScale, 0, 0));
	fogCoord.w *= (1 - (noise.r - RChannelOffset) * ceil(NoiseSpeed));*/
}
...
void HeightApplyFog(float4 fogCoord, inout float3 color)
{
	...
	float3 worldPos = fogCoord.xyz;
	float3 worldToCameraDir = normalize(_WorldSpaceCameraPos - worldPos);
	float4 noise = tex2D(_NoiseTex, float2(worldPos.x / NoiseScale + _Time.y * NoiseSpeed, worldPos.y / NoiseScale));
	fogCoord.w *= (1 - (noise.r - RChannelOffset) * ceil(NoiseSpeed));
...
//	#define UNITY_APPLY_FOG(coord,col) TL_APPLY_FOG(coord,col.rgb)
//#endif

void HeightApplyFog_ForwardAdd(float4 fogCoord, inout float3 color)
{
    //float3 worldToCameraDir = fogCoord.xyz;
    float3 worldPos = fogCoord.xyz;
    float3 worldToCameraDir = normalize(_WorldSpaceCameraPos - worldPos);
    ...
    // float3 fogColor = FogColor2.rgb /** saturate(worldToCameraDir.y * 5 + 1)*/ /*+ FogColor*/ + dirColor;
    float3 fogColor = fixed3(0,0,0);

    color = (lerp(color, ((color * (1 - fogCoord.w)) + fogColor), fogCoord.w));
}

#define TL_APPLY_FOG_FORWARDADD(coord,col)	HeightApplyFog_ForwardAdd(coord,col)

void WaterHeightApplyFog(float4 fogCoord, inout float3 color)
{
	float3 worldToCameraDir = fogCoord.xyz;
	//half3 lightDir = normalize(-_FogRefLight);
	half3 lightDir = _FogRefLight;
	float dirFactor = clamp(dot(-worldToCameraDir, lightDir), 0.0, 1.0);
	dirFactor = dirFactor * dirFactor;
	half3 dirColor = dirFactor * FogColor3.xyz;
...
float GetGrayColor(float3 color)
{
	...
	{
		return 1 - (color.r * 0.2126 + color.g * 0.7152 + color.b * 0.0722);
	}
}

float3 GetNormalByGray(sampler2D tex, float2 uv, float2 texelSize)
{
	float2 deltaU = float2(texelSize.x * _DeltaScale, 0);
	float center = GetGrayColor(tex2D(tex, uv).rgb);
	float h1_u = GetGrayColor(tex2D(tex, uv - deltaU).rgb);
	float h2_u = GetGrayColor(tex2D(tex, uv + deltaU).rgb);
	////µ±ǰֵ
	//float l1 = GetGrayColor(tex2D(tex, uv - 2 * deltaU).rgb);
	//float l2 = GetGrayColor(tex2D(tex, uv - 3 * deltaU).rgb);
	//float r1 = GetGrayColor(tex2D(tex, uv).rgb);
	//float r2 = GetGrayColor(tex2D(tex, uv + deltaU).rgb);
	////h1_u = (l1 + l2 + r1 + r2)/4;
	//	
	//l1 = GetGrayColor(tex2D(tex, uv - deltaU).rgb);
	//l2 = GetGrayColor(tex2D(tex, uv + 2 * deltaU).rgb);
	//r2 = GetGrayColor(tex2D(tex, uv + 3 * deltaU).rgb);
	////h2_u = (l1 + l2 + r1 + r2) / 4;
	//// float3 tangent_u = float3(1, 0, (h2_u - h1_u) / deltaU.x);
	float3 tangent_u = float3(deltaU.x, 0, _HeightScale * (h2_u - h1_u));
	//float3 tangent_u = float3(deltaU.x, 0, _HeightScale * ((h2_u + h1_u + center)/3 - center));
		
		
	float2 deltaV = float2(0, texelSize.y * _DeltaScale);
	float h1_v = GetGrayColor(tex2D(tex, uv - deltaV).rgb);
	float h2_v = GetGrayColor(tex2D(tex, uv + deltaV).rgb);
	//
	//l1 = GetGrayColor(tex2D(tex, uv - 2 * deltaV).rgb);
	//l2 = GetGrayColor(tex2D(tex, uv - 3 * deltaV).rgb);
	//r2 = GetGrayColor(tex2D(tex, uv + deltaV).rgb);
	////h1_v = (l1 + l2 + r1 + r2) / 4;
	//l1 = GetGrayColor(tex2D(tex, uv - deltaV).rgb);
	//l2 = GetGrayColor(tex2D(tex, uv + 2 * deltaV).rgb);
	//r2 = GetGrayColor(tex2D(tex, uv + 3 * deltaV).rgb);
	////h2_v = (l1 + l2 + r1 + r2) / 4;
	//// float3 tangent_v = float3(0, 1, (h2_v - h1_v) / deltaV.y);
	float3 tangent_v = float3(0, deltaV.y, _HeightScale * (h2_v - h1_v));
	//float3 tangent_v = float3(0, deltaV.y, _HeightScale * ((h2_v + h1_v + center)/3 - center));
	//float3 normal = normalize(cross(tangent_v, tangent_u));
	float3 normal = normalize(cross(tangent_u, tangent_v));
	/*normal.xy *= _NoramlScale;
	...
	//normal = normalize(normal);
	//normal.xy *= _BumpDir;
	//normal = clamp(normal - 0.1, float3(0, 0, 0), float3(1, 1, 1)) + 0.1;
	return normal;
}
